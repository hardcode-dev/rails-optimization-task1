...
Для защиты от потери достигнутого прогресса при дальнейших изменениях программы был добавлен perfomance spec, который проверяет, что код выполняется меньше чем за 0.005 с файлов входных данных на 1000 строк. (Пришлось сделать файл гораздо больше, т.к на меньшем объёме данных не было разницы в скорости выполнения, нужно подумать над тем, чтобы уменьшить его до 100 строк)
