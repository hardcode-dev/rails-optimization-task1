...
Удалось улучшить метрику системы с *`0.188  (± 0.0%) i/s -      1.000  in   5.312967s` (13_000 строк, 520Кб), до `12.796  (± 3.3%) i/s -     63.000  in   5.013379s` (ускорение: 6700%)*.

Требуемый файл `data_large.txt` был обработан за `24.77 sec.`, что укладывается в заданный бюджет `30 sec.`

Самым оптимальным инструментом в поиске главных точек роста оказался `stackprof` 

## Защита от регрессии производительности
Для защиты от потери достигнутого прогресса при дальнейших изменениях программы защищает тест `task-1-benchmark-test` измеряющий кол-во итераций в сек.
