...
- Есть улучшения
  ...
  | ... | ... |
  |  N  | O(N) |
- исправленная проблема не перестала быть главной точкой роста, но уменьшилась до 17.09%
...
Так как алгоритм выполенния O(N), то можно выполнить подсчет времени выполнения для 3250940 строк,
(3250940 / 1500) * 0.13 = ~28,17сек.
...
```
где идет проверка на время выполнения программы 
