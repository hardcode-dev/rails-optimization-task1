...
Процент загрузки процессора при обратке 100_000 строк файла не больше 30%
...
- Выставлены границы
  - 100_000 строк: 1 секунда выполнения.
  - Логарифмическая сложность: константная
...
### находка №4
 ...
 - Оптимизация
    ...
    - Заменил each для перебора users на map, чтобы не формировать новый массив 
 - Изменение метрики
...
### находка №7
  ...
  - Оптимизация
     ...
     - Заменил merge с присвоением к переменной на merge! для report хеша
     - Убрал дублирущие map циклы для потомков each цикла
     - Перенес генерацию всей инфомрации по пользователю в один блок.
     - Благодаря переносу в один блок получилось убрать merge! из each цикла для report хэша
  - Изменение метрики
...
### находка №10
  ...
  - Оптимизация
    ...
    - Заменил File.read на File.readlines
...
### находка №15
  ...
  - Оптимизация
       ...
       - Удалил преобразование даты с помощью Date, т.к. дата поступает корректно. 
...
Удалось улучшить метрику системы с *того, что у вас было в начале, до того, что получилось в конце* и уложиться в 
заданный бюджет.
...
программы *о performance-тестах, которые вы написали*
