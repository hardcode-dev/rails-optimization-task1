...
Однако, Array#each вызывается в нескольких местах программы. Попробуем сформировать отчет ruby-prof в GraphHtml
Из отчета следует, что 85% времени работает метод Array#each вызванный из Object#work, строка программы 59. 

Рассмотрим это место программы. Тут мы видем, как целиком прочитанный в file_lines файл обрабатывается методом each.
Попробуем читать и обрабатывать файл построчно. Внесем изменения в код, 
исходное значение метрики оптимизации - 47 секунд (файл 5mb)
