...
### Ваша находка №6
**Точка роста**
* воспользовался ruby-prof-callstack

**Решение**
* Вот, что думаю...
  метод парсинга дат вроде как впринципе медленный, не полезу;
  ...
  Перестал использвоать users_objects, убрал модель User(если уж и что-н такое надо будет, то скорее буду через struct), но решил вернуть блок.. в методом будет не удобно работать
  Прогнал large_data файл, смог обработать за 43.790900sec... уже близко ))

**Изменение метрики**
* новое значение  0.012775sec
* улучшение ~1.25 раза

**Изменение в отчете профилировщика**
```bigquery
100.00% (100.00%) [global]# [1 calls, 1 total]
99.93% (99.93%) Parser#work [1 calls, 1 total]
61.61% (61.65%) <Class::IO>#foreach [1 calls, 1 total]
29.70% (48.20%) Parser#parse_session [2742 calls, 2742 total]
19.07% (64.22%) Parser#parse_date [2742 calls, 2742 total]
11.03% (57.84%) <Class::Date>#strptime [2742 calls, 2742 total]
7.15% (11.60%) String#split [3250 calls, 3250 total]
20.08% (20.10%) Parser#collect_stats_from_users [1 calls, 1 total]
20.07% (99.96%) Array#each [1 calls, 2 total]
5.41% (26.96%) Array#map [1524 calls, 1525 total]
9.42% (9.42%) <Class::IO>#write [1 calls, 1 total]
```
**На подумать:**
- надо посмотреть м.б. есть методы чтения файла быстрее foreach(IO.readlines?)
- есть что-н быстрее split?(потестить lines)
- почему такой разброс при замерах бывает?...
- добавил прогрессбар, но с ним не влажу в бюджет(пока закомментил)

## Результаты
В результате проделанной оптимизации наконец удалось обработать файл с данными.
Удалось улучшить метрику системы с 0.174235sec до 0.012775sec и уложиться в заданный бюджет.


## Защита от регрессии производительности
Для защиты от потери достигнутого прогресса при дальнейших изменениях программы написал два теста:
* на прогон тестовых данных
* на прогон large_data из задания
