...
describe "UserWork" do
  let(:raw_data) { File.read("./spec/fixtures/input_data.txt") }
  ...
  describe ".work" do
    it "return correct data" do
      allow(File).to receive(:read).with("./test.txt").and_return(raw_data)

      expect(work("./test.txt")).to eq(expected_result)
    end
  end
end