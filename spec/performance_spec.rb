...
describe 'Perfomance' do
  ...
  context 'works under 3ms for 100 strings of data' do
    ...
    it {
      prepare_data(size) do |filename|
        expect do
          work(filename)
        end.to perform_under(3).ms.warmup(2).times.sample(10).times
      end
    }
  end
  ...
  context 'works under 0.05s for 1000 strings of data' do
    ...
    it {
      prepare_data(size) do |filename|
        expect do
          work(filename)
        end.to perform_under(10).ms.warmup(2).times.sample(10).times
      end
    }
  end
  ...
  context 'works exponential' do
    xit do
      ...
      end.to perform_linear.in_range(10, 10_000)
    end
  end
end
