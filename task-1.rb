...
def work(file_name = 'data.txt')
  ...
  browsers = sessions.map { |s| s['browser'].upcase }.uniq.sort
  ...
  users_objects = collect_user_objects(users, sessions)
...
def collect_user_objects(users, sessions)
  users.map { |user| User.new(attributes: user, sessions: sessions[user['id']]) }
end
