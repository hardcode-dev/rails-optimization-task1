...
def work(file_name)
  ...
  file_progressbar = progress_bar(file_lines)
  ...
  session_progressbar = progress_bar(sessions)

  sessions.each do |session|
    browser = session['browser']
    uniqueBrowsers += [browser] if uniqueBrowsers.all? { |b| b != browser }
    session_progressbar.increment
  end
  ...
  users_progress_bar = progress_bar(users)
  users.each do |user|
    attributes = user
...
def progress_bar(object)
  ProgressBar.create(total: object.count, format: '%a, %J, %E %B')
end
