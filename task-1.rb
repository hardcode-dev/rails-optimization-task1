...
def work(filename)
  ...
  file_lines.each do |line|
    cols = line.split(',')
    identifier = cols.first
    if identifier == 'user'
      users << parse_user(line)
    elsif identifier == 'session'
  ...
  # Хоть раз использовал IE?
  collect_stats_from_users(report, users_objects) do |user|
    { 'usedIE' => user.sessions.map { |s| s['browser'].upcase }.any? { |b| b =~ /INTERNET EXPLORER/ } }
  end

  # Всегда использовал только Chrome?
  collect_stats_from_users(report, users_objects) do |user|
    { 'alwaysUsedChrome' => user.sessions.map { |s| s['browser'].upcase }.all? { |b| b =~ /CHROME/ } }
  end
