...
describe 'parser' do
  ...
  it 'works' do
    work
    expect(File.read('result.json')).to eq(expected_json)
  end

  it 'work faster than 30sec for large file' do
    expect { work('data_large.txt') }.to perform_under(30).sec.sample(5).times
  end

  it 'progress linear' do
    expect { |n| work("data_#{n}k.txt") }.to perform_linear.in_range(1, 256)
  end
end
