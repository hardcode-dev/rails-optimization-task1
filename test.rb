...
class FuctionalTest < Test::Unit::TestCase
  ...
  def setup
    ...
    File.write('data.txt', INPUT_DATA)
  end

  def test_result
    work('data.txt')
    assert_equal JSON.parse(EXPECTED_RESULT), JSON.parse(File.read('result.json'))
  end
end

class BenchmarkTest < Test::Unit::TestCase
  def test_result
    assert_equal Benchmark.realtime { work('data100000.txt') } < 1, true
  end
end
